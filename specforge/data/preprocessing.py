...
def preprocess_vlm_conversations(
    ...
    # Support both single-image ("image" column) and multi-image ("images" column) datasets.
    ...
    for i in range(num_examples):
        ...
        # Use per-row system prompt if present, otherwise fall back to template
        ...
        # Build messages, inserting image content blocks where they appear
        for sentence in source:
            role = sentence["role"]
            content = sentence.get("content", "")

            # If content is already a list of content blocks (multi-modal format),
        ...
        # Allow examples with no images (text-only tool delegation turns)
        if image_inputs is None:
            image_inputs = []

        # Build processor kwargs, only pass images/videos if non-empty
        proc_kwargs = dict(
            text=[conversation],
        ...
        encoding = processor(**proc_kwargs)
...
def build_eagle3_dataset(
    ...
    if is_vlm:
        batch_size = (
            10  # reduce batch size for multi-image VLM datasets to avoid PyArrow offset overflow
        )
    else:
