...
def parse_arguments():
    ...
    model_group.add_argument(
        "--raw-input-output",
        action="store_true",
        help="If set, preprocess the prompt with the chat template and output raw input/output pairs instead of chat format."
    )
...
def call_sglang(
    ...
    if args.raw_input_output:
        # Render the prompt using the chat template, then send as raw prompt
        from transformers import AutoTokenizer
        import re
        tokenizer = AutoTokenizer.from_pretrained(args.model, trust_remote_code=True)
        messages = data["conversations"]
        # Use the tokenizer's chat template to render the prompt
        prompt = tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)
        # Call the completions endpoint with the raw prompt
        try:
        ...
        # Concatenate all assistant turns in the output, preserving order and special tokens
        # This regex matches all <|start|>assistant...<|end|> blocks (and similar)
        assistant_blocks = re.findall(r"<\|start\|>assistant.*?<\|end\|>", response_text, re.DOTALL)
        if assistant_blocks:
            output = "".join(assistant_blocks)
        else:
...
def main():
    ...
    with (
        ...
        def extract_prompt_output(conversations):
            system_msg = next((msg["content"] for msg in conversations if msg["role"] == "system"), "")
            user_msg = next((msg["content"] for msg in conversations if msg["role"] == "user"), None)
            assistant_msg = next((msg["content"] for msg in conversations if msg["role"] == "assistant"), None)
            ...
            while len(waiting_queue[server_address]) >= args.concurrency:
                ...
                for req_future in waiting_queue[server_address]:
                    if req_future.done():
                        ...
                        if args.raw_input_output:
                            if regen_data["status"] == "success":
                                output_file_handle.write(
                                    json.dumps({"prompt": regen_data["prompt"], "output": regen_data["output"]}, ensure_ascii=False) + "\n"
                                )
                                success_samples += 1
                            else:
                            ...
                            if regen_data["status"] == "error":
                                ...
                                prompt, output = extract_prompt_output(regen_data.get("conversations", []))
                                if prompt is not None and output is not None:
                                    ...
                                    error_file_handle.write(
                                        json.dumps({"status": "error", "error": "Missing user or assistant message", "data": regen_data}, ensure_ascii=False) + "\n"
                                    )
                                    error_samples += 1
